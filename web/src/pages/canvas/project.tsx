...
import { captureVideoPoster, getDataUrlByteSize } from "@/lib/image-utils";
...
function InfiniteCanvasPage() {
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            ...
            if (node.type === CanvasNodeType.Video) {
                ...
                addAsset({ kind: "video", title: node.metadata?.prompt?.slice(0, 24) || "画布视频", coverUrl: await captureVideoPoster(node.metadata.content), tags: [], source: "Canvas", data: { url: node.metadata.content, storageKey: node.metadata.storageKey, width: node.width, height: node.height, bytes: node.metadata.bytes || 0, mimeType: node.metadata.mimeType || "video/mp4" }, metadata: { source: "canvas", nodeId: node.id, prompt: node.metadata?.prompt } });
        ...
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                const generationMetadata = savedImageMetadata?.generationType
                    ? { generationType: savedImageMetadata.generationType, model: generationConfig.model, size: generationConfig.size, quality: generationConfig.quality, imageResponseFormat: generationConfig.imageResponseFormat, ...(generationConfig.background ? { background: generationConfig.background } : {}), count: savedImageMetadata.count || 1, references: savedImageMetadata.references }
    ...
    const insertAssistantImage = useCallback(
        async (image: CanvasAssistantImage) => {
            ...
            if (!sourceUrl) {
                message.error("图片内容已丢失，请重新选择素材");
                return;
            }
            const storedImage = await uploadImage(sourceUrl);
...
async function hydrateCanvasImages(nodes: CanvasNodeData[]) {
    return Promise.all(
        nodes.map(async (node) => {
            ...
            if (node.metadata?.storageKey) return { ...node, metadata: { ...node.metadata, content: await resolveImageUrl(node.metadata.storageKey, "") } };
...
async function hydrateAssistantImages(sessions: CanvasAssistantSession[]) {
    const hydrateItem = async <T extends { dataUrl?: string; storageKey?: string }>(item: T) => {
        if (item.storageKey) return { ...item, dataUrl: await resolveImageUrl(item.storageKey, "") };
