...
+ [修复] 长片视频失败时解析 Agnes 与兼容接口返回的 `msg`、`message`、`detail` 和嵌套错误，在镜头卡片中显示真实失败原因。
+ [新增] 长片工作台新增场景圣经与资源库场景图入口，LLM 解析会统一角色、场景及逐镜归属，首帧按文生图/图生图结构组装提示词并携带本镜人物与场景参考图以提升跨镜一致性。
+ [调整] 长片角色圣经从资产库引入时绑定参考图，出首帧/视频按本镜出场注入参考图以保持人物一致；首帧远程入库走同源代理，修复 Agnes platform-outputs CORS。
+ [调整] 长片「解析导入」改为一律由 LLM 结构化任意格式剧本，移除 Markdown/JSON/纯文本本地启发式解析。
+ [优化] 长片工作台重构为固定阶段轨道与逐镜流水线，合并分镜编辑和媒体审阅并精简重复操作。
+ [修复] 长片解析导入丢弃第一镜前的前言块，不再产生空白分镜。
+ [调整] 长片角色数据仅保留结构化 `characters`，移除 `characterBible` 旧字段及回退兼容。
+ [修复] 长片角色圣经外貌改为可自动换行的多行输入；LLM 辅助撰写只流式写入剧本区，去掉重复预览。
+ [修复] 长片单镜出首帧/出视频各自独立 abort 与 busy，互不打断其它镜头任务。
+ [修复] 长片分镜解析支持「镜头一 / 分镜2 / 【镜头1】」等标题，可正确导入后续镜头。
+ [修复] 长片「解析资产库图片」前将 blob 图转为 base64 data URL，避免接口无法加载 IMAGE 报 500。
+ [新增] 长片角色圣经支持从资产库选图，用 LLM 解析人物并导入角色表；风格圣经支持预设场景与 LLM 随机/按场景生成。
+ [修复] 长片工作台内容过长时可纵向滚动；单镜时长默认 5s、可在分镜表/结果卡按镜设 1–18s，出视频按本镜 durationSec 提交而非统一 18s。
+ [调整] 长片分镜与视频数量不再预设上限（取消默认 3 镜与解析截断），以实际导入/添加的分镜为准。
+ [调整] 长片工作台将分镜编辑与分镜结果分区展示，每镜首帧/尾帧/视频以独立结果卡片呈现。
+ [优化] 长片 `script.ts` 去掉硬编码指令套话，生成提示词仅拼接风格/角色/镜头原文；结构化解析与出场匹配保持纯数据逻辑。
+ [调整] 长片去掉内置 AI 拆解提示词，改为「LLM 辅助撰写」扩写剧本，并由 LLM「解析导入」结构化分镜。
+ [调整] 长片风格圣经强制注入全部首帧与视频生成；角色圣经支持多人结构化条目，分镜可指定出场并在多→单人镜中排除其他角色。
+ [调整] 长片工作台拆分「出首帧 / 出视频」阶段：分镜完成后可「批量出首帧」或单镜重出，审帧后再批量出视频；流程条与图片/视频模型选择同步。
+ [新增] 新增「长片工作台」：支持剧本导入、LLM 辅助撰写、首尾帧槽位、Agnes 单镜与批量生成、链式尾帧衔接与 ffmpeg.wasm 成片拼接导出。
+ [新增] 接入浏览器端 ffmpeg.wasm，支持视频首尾帧抽取与多段硬切拼接。
+ [调整] Agnes 视频关键帧支持显式首帧 / 尾帧入参，并在设置中标明参考图顺序约定。
...
+ [调整] Agnes Video 设置改为分辨率档位与比例入口；默认不强制 `mode`/`width`/`height`，关键帧与可选参数按文档提交；结果优先读取 `metadata.url`。
